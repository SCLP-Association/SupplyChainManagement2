...
def main():
    ...
    for i in range(len(Y)):
        for j in range(len(Y[0])):
            Y[i][j] = solver.IntVar(0.0, infinity, 'Y_'+str(i)+'_'+str(j))
...
[3491.8, 2431, 3876.6, 2269.8, 4997.2, 3140.8, 2555.8, 2888.6, 4417.4, 5990.4, 1071.2, 5262.4]]
    ...
    C_w = [totalDemand*0.25,totalDemand*0.25,totalDemand*0.25,totalDemand*0.25,totalDemand*0.25]
    #print(X[1][3])
    #print(Y)
    #print(Y[1][3])
    ...
    for i in range(Y_x):
        solver.Add(sumy(Y, col=i) == Demand[i])
    #Y[0][i]+Y[1][i]+Y[2][i]+Y[3][i]+Y[4][i]
    #print(sumy(D_f_w,col = 2))

    for i in range(Y_y):
        solver.Add(sumy(X, col=i) == sum(Y[i]))
